...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \item Memory: large, external, referred to by address
    \end{itemize}
    \item Instructions affect and/or use state
    \begin{itemize}
        \item Add a constant to a register, subtract two registers, write to a memory location, jump to a memory location if a condition is true, etc.
    \end{itemize}
    \vs
    \pause
...
\begin{itemize}
    ...
    \vs \vs \vs
    \pause
    \item Fetch - Decode - Execute Model
    \begin{itemize}
...
magicNumber:               # int *magicNumber;
  .long 0xdeadbeef         # *magicNumber = 0xdeadbeef;
\end{gascode*}
\end{frame}
...
\begin{itemize}
  \item {\ttfamily call <label>}
  \item {\ttfamily call} pushes a return address onto the stack, then jumps to {\ttfamily <label>}
...
\begin{frame}[fragile,t]
\mvs
\frametitle{99 Bottles of Beer on the Wall (99\_botttles\_of\_beer.S)}
...
\begin{frame}[fragile,t]
\mvs
\frametitle{Simple Hexdump (example-14-hexdump.S)}
\begin{gascode}
.section .text
.global _start
_start:
...
\begin{frame}[fragile,t]
\mvs
\frametitle{Simple Hexdump (example-14-hexdump.S) Continued}
\begin{gascode*}{textsize=\textsize{9}{8}}
    read_loop:
...
\begin{frame}[fragile,t]
\mvs
\frametitle{Simple Hexdump (example-14-hexdump.S) Runtime}
\begin{textcode}
...
\section{Advanced Topic 16: Comparisons with Atmel AVR and ARM Cortex-M3}
